...
public class EditarLeadAction extends DHTMLXFormAction {
...
/*	public void saveByName(DhtmlxForm f) throws Exception {
		LeadForm form = (LeadForm) f;
		LeadVO lead = new LeadVO();
		lead.setIdLead(form.getIdLead());
		...
		if (!StringUtil.isNullOrBlank(lead.getIdLead())) {
			...
			comercial.setNombre(form.getComercial());			
			lead.setComercial(comercial);
			DistribuidorVO distribuidor = new DistribuidorVO(null, form.getDistribuidor());
			lead.setDistribuidor(distribuidor );
			EmpresaVO empresa = new EmpresaVO();
			empresa.setNombre(form.getEmpresa());
			lead.setEmpresa(empresa );
			lead.setEstado(estado);
			lead.setMarketingActivity(marketingActivity);
			ResponsableVO responsable = new ResponsableVO(null, form.getResponsable());
			lead.setResponsable(responsable);
			ServicioVO servicio = new ServicioVO(null, form.getServicio());
			lead.setServicio(servicio );			
			CanalVO canal = new CanalVO(null, form.getCanal());
			lead.setCanal(canal);			
			
			getLeadsService().insertByName(lead);
		}
