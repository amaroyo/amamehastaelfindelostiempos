...
public class DescargarTodosDiariosReflexivosAlumnosAction extends MrmAction {
	...
	public ActionForward process(ActionMapping mapping, ActionForm form,
		...
		if(listUsuarioPortafolio != null){
			for (UsuarioPortafolioVO up : listUsuarioPortafolio) {
				...
				diario.setIdPortafolio(up.getIdPortafolio());
				//Lista con todos los casos clinicos de un portafolio
				List<DiarioReflexivoVO> diariosPortafolio =  getDiariosReflexivosService().findAllByPortafolio(diario);
				...
				if(diariosPortafolio != null){
					for (DiarioReflexivoVO d : diariosPortafolio) {	
						if(d.getDiarioReflexivo()!=null){
							ZipEntry subEntry = new ZipEntry(d.getNombre());
	...
	*/
		...
		try{
			...
			String nmbreArchivo = "Diarios_Reflexivos_de_" + a.getCodigo() + "_Curso_" + anyoAcademico;
