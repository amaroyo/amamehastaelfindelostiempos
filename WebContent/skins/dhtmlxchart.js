			...
			if (data.indexOf("->")!=-1){
				...
				switch(data[0]){
					...
					case "http": 	//load from external file
						data = new dhtmlx.ajax().sync().get(data[1],{uid:(new Date()).valueOf()}).responseText.replace(/\"/g,"\\\"");
						break;
					default:
...
dhtmlx.DataStore.prototype={
	...
	getRange:function(from,to){		
		if (!arguments.length){
			...
			var max_row = (this.max || this.max === 0)?this.max:Infinity;
			//if indexes not provided - return all visible rows
			from = this.min||0; to = Math.min(max_row,(this.dataCount()-1));
		} else{
	...
	remove:function(id){
		...
		if (this._tree_store)
			dhtmlx.extend(this.get(obj.parent).item, dhtmlx.PowerArray).remove(id);
...
/* DHX INITIAL FILE 'D:\web server\www\work\!!svn_dhtmlx\main\dhtmlxCore/sources//chart.js'*/
