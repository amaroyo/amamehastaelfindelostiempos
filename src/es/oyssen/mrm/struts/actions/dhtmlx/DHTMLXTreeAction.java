...
public abstract class DHTMLXTreeAction extends MrmAction {
		...
		if (!StringUtil.isNullOrBlank(f.getNativeeditor_status())) {
			...
			try {
				if (f.getNativeeditor_status().equals("inserted")) {
					...
					delete(f);					
				} else if (f.getNativeeditor_status().equals("draganddroped")) {
					f.setOperation("draganddrop");
					draganddrop(f);
				} else if (f.getNativeeditor_status().equals("dndgridtotreeop")) {
					f.setOperation("dndgridtotree");
					dndgridtotree(f);
			...
			xml.append("</data>");
			writeTreeXML(response, xml.toString());
		} else {
			List lista = search(f);		
			writeTreeXML(response, parseListToXML(lista));
		}
		
		return null;
	}

	protected void writeTreeXML(HttpServletResponse response,
			String xmlTree) throws IOException,
			UnsupportedEncodingException {
		response.setContentType("text/xml");
		ServletOutputStream sos = response.getOutputStream();
		sos.write(xmlTree.getBytes("UTF-8"));
		sos.flush();
		sos.close();
	}
	...
	public abstract void draganddrop(DhtmlxForm f) throws Exception;
	
	public abstract void dndgridtotree(DhtmlxForm f) throws Exception;
	
	public abstract void saveCopia(DhtmlxForm f) throws Exception;
}
